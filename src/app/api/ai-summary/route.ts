...
export async function POST(request: Request) {
	try {
		...
		if (!response.ok) {
			...
			return Response.json({ error: `AI服务请求失败: ${response.status}` }, { status: 500 })
		}
		...
		if (!summary) {
			...
			return Response.json({ error: '未获取到总结内容' }, { status: 500 })
		}

		return Response.json({ summary })
	} catch (error: any) {
		console.error('AI summary error:', error?.message || error)
		return Response.json({ error: `生成总结失败: ${error?.message || '未知错误'}` }, { status: 500 })
	}
}
